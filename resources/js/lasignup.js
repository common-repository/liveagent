...
jQuery(function($) {
	...
	var replacePlaceholder = function() {
		...
		if ($('#configOptionLevel').is(':checked')) {
			result += "%%level%%";
		}

		$('textarea#la-buttons_buttoncode').val([widgetCode.slice(0, pos), result, widgetCode.slice(pos)].join(''));
		$('.SaveWidgetCode').show();
	}
	
	$('textarea#la-buttons_buttoncode').on('change', function() {
		$('.SaveWidgetCode').show();
	});
